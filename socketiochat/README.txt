Chat application that uses Socket.io for bi-directional communication between the server and the client.

To run:
Ensure Node.js is installed and also install nodemon ("npm install nodemon")
Clone the repo
Change to socketiochat directory
Run `npm install`  //to install the dependencies i.e. node-modules
Run `nodemon` or `node server` if nodemon not installed

To Demo:
Open 2 browser windows at `http://localhost:3000`
Login with any name
Sent messages should appear in the alternate windows

General Information

The app is based on one created in this tutorial:
